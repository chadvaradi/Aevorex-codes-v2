...
def ensure_datetime_index(df: pd.DataFrame, function_name: str = "caller") -> pd.DataFrame | None:
...
def validate_ohlcv_dataframe(df: pd.DataFrame, context: str = "") -> pd.DataFrame | None:
    ...
    if df_indexed is None or df_indexed.empty:
        logger.error(f"{log_prefix} DataFrame preparation failed or resulted in empty DF.")
        return None
    ...
    if not required_cols.issubset(df_indexed.columns):
        ...
        logger.error(f"{log_prefix} Missing required OHLCV columns (lowercase): {missing_cols}. Found: {df_indexed.columns.tolist()}")
        return None
