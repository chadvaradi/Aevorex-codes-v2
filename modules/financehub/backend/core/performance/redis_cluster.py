...
class RedisClusterManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            logger.info(f"[RedisCluster] Initializing cluster with {len(self.cluster_nodes)} nodes")
    ...
    async def set_with_compression(
        self, 
        key: str, 
        value: Any, 
        ttl: int = 3600
        ...
        try:
            ...
            logger.debug(f"[RedisCluster] SET {key} (size: {len(processed_data)} bytes)")
            return True
            ...
            for key, data in zip(keys, values):
                if data:
                    try:
                        ...
                        logger.warning(f"[RedisCluster] Failed to deserialize {key}: {e}")
            ...
            for i, node in enumerate(nodes):
                try:
                    ...
                    info[f"node_{i}"] = {
                        ...
                        "total_commands_processed": node_info.get("total_commands_processed"),
                        "keyspace_hits": node_info.get("keyspace_hits"),
                        "keyspace_misses": node_info.get("keyspace_misses")
                    }
                except Exception as e:
                    logger.warning(f"[RedisCluster] Failed to get info from node {i}: {e}")
