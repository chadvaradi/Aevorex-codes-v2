...
class DatabasePool:
    ...
    async def initialize(self) -> None:
        ...
        try:
            logger.info(f"[DatabasePool] Initializing pool (min: {self.min_size}, max: {self.max_size})")
    ...
    async def get_connection(self) -> AsyncGenerator[asyncpg.Connection, None]:
        ...
        try:
            ...
            logger.warning("[DatabasePool] Pool exhausted, waiting for available connection")
            raise
        ...
        *args, 
        ...
        try:
            ...
            if elapsed > 1.0:  # Log slow queries
                logger.warning(f"[DatabasePool] Slow query ({elapsed:.2f}s): {query[:100]}...")
    ...
    async def execute_transaction(
        ...
        try:
            ...
            logger.debug(f"[DatabasePool] Transaction completed ({elapsed:.2f}s, {len(queries)} queries)")
            return results
...
timescale_pool = TimescaleDBPool(
    dsn=settings.TIMESCALE_URL,
    min_size=5,
    max_size=20
) if hasattr(settings, 'TIMESCALE_URL') else None 