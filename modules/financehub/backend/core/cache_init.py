...
async def initialize_cache_service(cls: type[CacheService]) -> CacheService:
    ...
    try:
        ...
        logger.critical(f"{MODULE_PREFIX} FATAL: Could not connect to Redis: {e}", exc_info=True)
        raise RuntimeError(f"Failed to connect to Redis backend: {e}") from e
    except RedisError as e:
        logger.critical(f"{MODULE_PREFIX} FATAL: Redis error during initialization: {e}", exc_info=True)
        raise RuntimeError(f"Redis backend initialization failed: {e}") from e
    except AttributeError as e:
        logger.critical(f"{MODULE_PREFIX} FATAL: Missing Redis configuration in settings: {e}", exc_info=True)
        raise RuntimeError(f"Missing Redis configuration: {e}") from e
    except Exception as e:
        logger.critical(f"{MODULE_PREFIX} FATAL: Unexpected error during initialization: {e}", exc_info=True)
