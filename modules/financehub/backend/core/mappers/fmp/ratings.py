...
def map_fmp_raw_ratings_to_rating_points(
    ...
    for i, raw_rating_item in enumerate(ratings_list):
        ...
        try:
            ...
            logger.warning(f"{item_log_prefix} Skipping rating due to validation error: {e}. Raw data: {raw_rating_item}")
            skipped_item_count += 1
            
    map_duration = time.monotonic() - map_start_time
    logger.info(f"{log_prefix} Mapped {len(processed_rating_points)} FMP rating items in {map_duration:.4f}s. Skipped {skipped_item_count} items.")
