...
from modules.financehub.backend.core.helpers import _clean_value, normalize_url, parse_timestamp_to_iso_utc
from modules.financehub.backend.core.mappers._mapper_base import logger, StandardNewsDict, DEFAULT_NA_VALUE

def map_fmp_item_to_standard(raw_item: dict[str, Any], api_source: str) -> StandardNewsDict | None:
    """
    ...
    try:
        ...
        standard_dict: StandardNewsDict = {
            ...
            "meta_data": {"fmp_original_symbol": raw_item.get("symbol", DEFAULT_NA_VALUE)}
        }
        return standard_dict

    except Exception as e:
        logger.error(f"{log_prefix} Unexpected error mapping FMP news item (URL: {raw_item.get('url', 'N/A')}): {e}", exc_info=True)
        return None
