...
try:
    ...
    cl.critical(f"FATAL ERROR: Cannot import from _mapper_base in _shared_mappers: {e_base}.", exc_info=True)
    raise RuntimeError(f"Shared mappers setup failed due to missing base module: {e_base}") from e_base
...
try:
    ...
    logger.error(f"Failed to import helper functions in _shared_mappers: {e_helpers}", exc_info=True)
...
try:
    ...
    logger.critical(f"FATAL ERROR: Failed importing internal mapper components in _shared_mappers: {e_internal}.", exc_info=True)
    raise RuntimeError(f"Shared mappers setup failed due to missing internal components: {e_internal}") from e_internal
except Exception as e_init_siblings:
    logger.critical(f"FATAL UNEXPECTED ERROR during sibling module import in _shared_mappers: {e_init_siblings}", exc_info=True)
    raise RuntimeError(f"Shared mappers failed during sibling initialization: {e_init_siblings}") from e_init_siblings
...
def map_raw_news_to_standard_dicts( raw_news_list: list[dict[str, Any]] | None, source_api_name: str ) -> list[StandardNewsDict]:
    ...
    for i, item in enumerate(raw_news_list):
        try:
            ...
            logger.error(f"{log_prefix} Error mapping item #{i+1}: {e}", exc_info=True)
            skipped += 1
            continue
            
    t_end = time.monotonic()
    logger.info(f"{log_prefix} Mapping done. Proc: {processed}, Skip: {skipped}. Took: {t_end - t_start:.4f}s") 
    return std_dicts
    ...
    for i, std_dict in enumerate(standard_news_dicts):
        ...
        try:
            ...
            if not url_value_raw or not isinstance(url_value_raw, (HttpUrl, str)):
                logger.warning(f"{item_log_prefix} Skipping: Missing or invalid URL type ({type(url_value_raw)}).")
                item_validation_errors += 1
                continue
            try:
                ...
                logger.error(f"{item_log_prefix} Skipping: Failed UUID generation: {e_uuid}.")
                item_validation_errors += 1
                continue
