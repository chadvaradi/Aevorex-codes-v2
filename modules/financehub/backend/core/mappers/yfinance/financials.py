...
from modules.financehub.backend.models import FinancialsData
from modules.financehub.backend.core.mappers._mapper_base import logger, safe_get
from modules.financehub.backend.core.helpers import parse_optional_int
...
_YFINANCE_LIABILITIES_KEYS: list[str] = ['Total Liabilities Net Minority', 'Total Liabilities', 'Total Liabilities Net Minority']

def get_latest_financial_values(df: pd.DataFrame | None, df_name: str, df_type_log: str, request_id: str, symbol: str = "N/A_SYMBOL") -> dict[str, Any]:
    """
    ...
    if not isinstance(df.columns, pd.DatetimeIndex):
        logger.warning(f"{inner_log_prefix} DataFrame columns are not a DatetimeIndex. Cannot reliably determine the latest period. Columns: {df.columns}")
        return results

    latest_column = df.columns.max()
    results['latest_report_date'] = latest_column.strftime('%Y-%m-%d')
    logger.debug(f"{inner_log_prefix} Identified latest report date: {results['latest_report_date']}")
        ...
        for item_key in item_keys:
            ...
            if value is not None:
                ...
                if parsed_value is not None:
                    ...
                    logger.debug(f"{inner_log_prefix} Found and parsed '{item_key}' as {result_key}: {parsed_value}")
                    break
        if results[result_key] is None:
            logger.debug(f"{inner_log_prefix} Could not find a valid value for {result_key} using keys: {item_keys}")
...
def map_financial_dataframes_to_financials_model(
    ...
    if not financial_dfs or not any(isinstance(df, pd.DataFrame) and not df.empty for df in financial_dfs.values()):
        logger.warning(f"{log_prefix} Input financial_dfs is None, empty, or contains no valid DataFrames. Cannot map to FinancialsData.")
        return None

    annual_financials = get_latest_financial_values(financial_dfs.get('financials_annual'), 'financials_annual', 'annual', request_id, symbol)
    quarterly_financials = get_latest_financial_values(financial_dfs.get('financials_quarterly'), 'financials_quarterly', 'quarterly', request_id, symbol)
    annual_balance = get_latest_financial_values(financial_dfs.get('balance_sheet_annual'), 'balance_sheet_annual', 'annual', request_id, symbol)
    quarterly_balance = get_latest_financial_values(financial_dfs.get('balance_sheet_quarterly'), 'balance_sheet_quarterly', 'quarterly', request_id, symbol)
    ...
    final_data = {
        ...
        "total_assets": annual_balance['latest_total_assets'] or quarterly_balance['latest_total_assets'],
        "total_liabilities": annual_balance['latest_total_liabilities'] or quarterly_balance['latest_total_liabilities'],
    ...
    try:
        ...
        logger.info(f"{log_prefix} Successfully mapped financial DataFrames to FinancialsData model.")
        return model
    except Exception as e:
        logger.error(f"{log_prefix} Pydantic validation failed for FinancialsData: {e}", exc_info=True)
        return None
