...
from modules.financehub.backend.models import EarningsPeriodData
...
def map_yfinance_financial_statement_to_earnings_periods(
    ...
    if not isinstance(statement_df, pd.DataFrame) or statement_df.empty:
        logger.debug(f"{log_prefix} Input DataFrame for '{report_period_type}' is invalid or empty. Returning empty list.")
        return []

    if not isinstance(statement_df.columns, pd.DatetimeIndex):
        logger.warning(f"{log_prefix} DataFrame for '{report_period_type}' does not have a DatetimeIndex. Columns: {statement_df.columns}. Cannot process.")
        return []
    ...
    for period_date in statement_df.columns:
        ...
        try:
            ...
            period_data = {
                ...
                "eps": parse_optional_int(eps_raw), # Should be float, but let's stick to int for now as per original
                "currency_code": currency,
                "period_type": report_period_type
            }

            earnings_periods.append(EarningsPeriodData(**{k:v for k,v in period_data.items() if v is not None}))
            logger.debug(f"{period_log_prefix} Successfully created EarningsPeriodData.")
            
        except Exception as e:
            logger.error(f"{period_log_prefix} Error processing period: {e}", exc_info=True)

    logger.info(f"{log_prefix} Mapped {len(earnings_periods)} '{report_period_type}' earnings periods.")
