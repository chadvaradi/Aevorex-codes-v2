...
from modules.financehub.backend.models import CompanyOverview
from modules.financehub.backend.core.helpers import _clean_value, parse_optional_float, parse_optional_int, parse_timestamp_to_iso_utc, normalize_url
from modules.financehub.backend.core.mappers._mapper_base import logger

def parse_ts_to_date_str(ts_val: Any, field_name: str, context_prefix: str) -> str | None:
    """Parse timestamp value to date string format."""
    if ts_val is None:
        return None
    parsed_iso = parse_timestamp_to_iso_utc(ts_val, context=f"{context_prefix}:{field_name}")
...
def map_yfinance_info_to_overview(
    ...
    if not info_dict:
        logger.warning(f"{log_prefix} Input yfinance 'info' dictionary is None or empty. Cannot map to CompanyOverview.")
        return None
    ...
    try:
        overview_data = {
            ...
            "full_time_employees": parse_optional_int(info_dict.get('fullTimeEmployees')),
            "market_cap": parse_optional_int(info_dict.get('marketCap')),
            "shares_outstanding": parse_optional_int(info_dict.get('sharesOutstanding')),
            ...
            "price_to_sales_ratio": parse_optional_float(info_dict.get('priceToSalesTrailing12Months')),
            ...
            "last_dividend_date": parse_ts_to_date_str(info_dict.get('lastDividendDate'), 'lastDividendDate', log_prefix),
            ...
            "fifty_two_week_high": parse_optional_float(info_dict.get('fiftyTwoWeekHigh')),
            "fifty_two_week_low": parse_optional_float(info_dict.get('fiftyTwoWeekLow')),
            "fifty_day_average": parse_optional_float(info_dict.get('fiftyDayAverage')),
            "two_hundred_day_average": parse_optional_float(info_dict.get('twoHundredDayAverage')),
        ...
        logger.info(f"{log_prefix} Successfully mapped yfinance 'info' to CompanyOverview model.")
        return model

    except ValidationError as e:
        logger.error(f"{log_prefix} Pydantic validation failed for CompanyOverview: {e}", exc_info=True)
        return None
