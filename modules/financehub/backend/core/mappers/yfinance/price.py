...
def map_yfinance_ohlcv_df_to_chart_list(
    ...
    if not isinstance(ohlcv_df, pd.DataFrame) or ohlcv_df.empty:
        logger.warning(f"{log_prefix} Input OHLCV DataFrame is invalid or empty. Cannot create chart list.")
        return None
    ...
    if not required_cols.issubset(df.columns):
        ...
        logger.error(f"{log_prefix} Missing required OHLCV columns after normalization: {missing_cols}. Cannot proceed.")
        return None
    ...
    if not isinstance(df_renamed.index, pd.DatetimeIndex):
        logger.error(f"{log_prefix} DataFrame index is not a DatetimeIndex. Cannot convert timestamps.")
        return None
    ...
    try:
        ...
        logger.info(f"{log_prefix} Successfully mapped OHLCV DataFrame to a list of {len(chart_list)} chart points.")
        return chart_list
        
    except Exception as e:
        logger.error(f"{log_prefix} An unexpected error occurred during final mapping: {e}", exc_info=True)
        return None
