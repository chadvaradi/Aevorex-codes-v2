...
from modules.financehub.backend.core.mappers._mapper_base import logger, StandardNewsDict, YFINANCE_NEWS_DEFAULT_SOURCE_NAME
from modules.financehub.backend.core.helpers import _clean_value, parse_timestamp_to_iso_utc, normalize_url
...
def map_yfinance_news_to_standard_dicts(
    ...
    if content_dict.get("publisher"):
        ...
        if raw_publisher_name:
            logger.debug(f"{log_prefix} Publisher not in 'content', using fallback '{source_field_publisher}'.")
    
    publisher = _clean_value(raw_publisher_name)
    logger.debug(f"{log_prefix} Publisher extracted from '{source_field_publisher}': '{publisher}'")
    ...
    return {
        ...
        "summary": summary_cleaned if summary_cleaned else f"Summary for '{title}' was not available.",
