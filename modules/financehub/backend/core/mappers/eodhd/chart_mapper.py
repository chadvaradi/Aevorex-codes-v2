# backend/core.ppers/eodhd/chart_mapper.py
# ==============================================================================
...
# --- Local EODHD Mapper Imports ---
try:
    from .helpers import preprocess_ohlcv_dataframe
    from .events_mapper import map_eodhd_splits_data_to_models, map_eodhd_dividends_data_to_models
except ImportError as e:
    logger.critical(f"FATAL ERROR: Cannot import local EODHD mappers: {e}", exc_info=True)
    raise RuntimeError(f"EODHD Chart mapper failed initialization: {e}") from e
...
def map_eodhd_data_to_chart_ready_format(
    ...
    processed_ohlcv_df = preprocess_ohlcv_dataframe(ohlcv_df, request_id, interval, log_prefix)
    ...
    for item in chart_list:
        ...
        # Rename OHLCV keys and ensure they exist
