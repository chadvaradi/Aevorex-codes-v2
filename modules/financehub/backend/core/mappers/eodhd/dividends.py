...
def map_eodhd_dividends_data_to_models(
    ...
    if not isinstance(dividends_df, pd.DataFrame) or dividends_df.empty:
        logger.info(f"{log_prefix} Input dividends_df is invalid or empty. Returning empty list.")
        return []

    dividend_models: list[DividendData] = []
    
    if 'date' not in dividends_df.columns or 'value' not in dividends_df.columns:
        logger.error(f"{log_prefix} Missing 'date' or 'value' column in DataFrame. Cannot process dividends.")
        return None
    ...
    for index, row in dividends_df.iterrows():
        try:
            ...
            if not dividend_date_str or dividend_value_raw is None:
                logger.warning(f"{log_prefix} Skipping dividend at index {index} due to missing data.")
                continue
            ...
            logger.error(f"{log_prefix} Error processing dividend row at index {index}: {e}", exc_info=True)
