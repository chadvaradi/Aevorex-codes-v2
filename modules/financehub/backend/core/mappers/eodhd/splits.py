...
from modules.financehub.backend.core.helpers import _clean_value
...
def map_eodhd_splits_data_to_models(
    ...
    if not isinstance(splits_df, pd.DataFrame) or splits_df.empty:
        logger.info(f"{log_prefix} Input splits_df is invalid or empty. Returning empty list.")
        return []
    ...
    if 'Date' not in splits_df.columns or 'Stock_Split' not in splits_df.columns:
        logger.error(f"{log_prefix} Missing 'Date' or 'Stock_Split' column in DataFrame. Cannot process splits.")
        return None
    ...
    for index, row in splits_df.iterrows():
        try:
            ...
            if not split_date_str or not split_ratio_str:
                logger.warning(f"{log_prefix} Skipping split at index {index} due to missing data.")
                continue
            ...
            logger.error(f"{log_prefix} Error processing split row at index {index}: {e}", exc_info=True)
