...
from modules.financehub.backend.utils.helpers_parser import parse_optional_float
...
def _preprocess_ohlcv_dataframe(
    ...
    if not isinstance(df_copy.index, pd.DatetimeIndex):
        try:
            ...
            logger.error(f"{log_prefix} Preprocessing failed: DataFrame index is not DatetimeIndex and conversion failed: {e_conv}.")
            return None
    else:
    ...
    if not required_cols.issubset(set(df_copy.columns)):
        ...
        logger.error(f"{log_prefix} Preprocessing failed: Missing critical UPPERCASE OHLC columns: {missing_required}.")
        return None
...
def map_eodhd_ohlcv_to_price_history_entries(
    ...
    for row in processed_df.itertuples(index=True, name='OHLCVRow'):
        try:
            ...
            logger.error(f"{log_prefix} Error processing row {row.Index}: {e}", exc_info=True)
