...
try:
    ...
    # This path should not be hit in production, indicates a structural problem
    import logging
    logging.basicConfig(level="CRITICAL")
    logging.getLogger(__name__).critical(f"FATAL ERROR: Cannot import helpers or preprocessing functions: {e}", exc_info=True)
    raise RuntimeError(f"EODHD OHLCV mapper failed initialization: {e}") from e
...
try:
    ...
    logger.warning("Could not import settings from config. Using dummy settings.")
    class DummySettings:
        class DummyEnv:
            NODE_ENV = 'production'
...
def map_eodhd_ohlcv_to_price_history_entries(
    ...
    processed_df = preprocess_ohlcv_dataframe(ohlcv_df, request_id, interval, log_prefix_base)
    ...
    if 'Volume' not in processed_df.columns:
        logger.error(f"{log_prefix} 'Volume' column missing after preprocessing, this should not happen.")
    ...
    for row in processed_df.itertuples(index=True, name='OHLCVRow'):
        ...
        try:
            ...
            low = parse_optional_float(getattr(row, 'Low', None), context=f"{point_log_prefix}:Low")
            c = parse_optional_float(getattr(row, 'Close', None), context=f"{point_log_prefix}:Close")
            adj_c = parse_optional_float(getattr(row, 'Adj_Close', None), context=f"{point_log_prefix}:AdjClose")
            ...
            entry_data = {
                'time': unix_ts_seconds, 'open': o, 'high': h, 'low': low, 'close': c,
                'adj_close': adj_c, 'volume': v
            }
    ...
    logger.info(f"{log_prefix} Pydantic mapping complete. Mapped: {processed_count}, Skipped: {skipped_count}.")
...
def map_eodhd_ohlcv_df_to_frontend_list(
    ...
    for row in processed_df.itertuples(index=True, name='OHLCVRow'):
        ...
        try:
            ...
            mapped_point = {'t': unix_ts_milliseconds, 'o': o, 'h': h, 'l': low, 'c': c, 'v': v}
    ...
    logger.info(f"{log_prefix} Frontend list mapping complete. Mapped: {processed_count}, Skipped: {skipped_count}.")
    return mapped_list 