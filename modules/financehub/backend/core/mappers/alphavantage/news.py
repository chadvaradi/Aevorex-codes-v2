...
from modules.financehub.backend.core.mappers._mapper_base import logger, StandardNewsDict
from modules.financehub.backend.core.helpers import _clean_value, parse_optional_float, normalize_url

def map_alphavantage_item_to_standard(raw_item: dict[str, Any]) -> StandardNewsDict | None:
    """
    ...
    try:
        ...
        if not title:
            ...
            return None
        if not url:
            logger.warning(f"{log_prefix} Skipping item: Invalid or missing 'url'. Original raw_url: {raw_url}")
            return None
        ...
        image_url: HttpUrl | None = normalize_url(raw_image_url, log_prefix=log_prefix) if raw_image_url else None
