...
from modules.financehub.backend.models import EarningsData, EarningsPeriodData
from modules.financehub.backend.core.mappers._mapper_base import logger
from modules.financehub.backend.core.helpers import parse_optional_float
...
def map_alpha_vantage_earnings_to_model(
    av_earnings_data: dict[str, Any] | None,
    request_id: str
) -> EarningsData | None:
    """
        ...
        for i, report_dict in enumerate(raw_list):
            ...
            try:
                ...
                if fiscal_date_obj is None:
                    logger.warning(f"{report_log_prefix} Skipping report due to missing or invalid fiscal date. Data: {report_dict}")
                    skipped_count += 1
                    continue
                ...
                if validated_model:
                    ...
                    skipped_count += 1
                    logger.warning(f"{report_log_prefix} Skipping report due to validation failure. Data: {period_data}")

            except Exception as e:
                logger.error(f"{report_log_prefix} Unexpected error processing report item: {e}", exc_info=True)
                skipped_count += 1
        
        if skipped_count > 0:
            logger.warning(f"{parent_log_prefix} Skipped {skipped_count}/{len(raw_list)} {report_type} reports due to errors.")
        
        return processed_list

    annual_reports = _process_av_report_list(annual_reports_raw, "annual", log_prefix)
    quarterly_reports = _process_av_report_list(quarterly_reports_raw, "quarterly", log_prefix)
    ...
    if validated_model:
        logger.info(f"{log_prefix} Successfully mapped Alpha Vantage data to EarningsData model for symbol '{symbol}' in {map_duration:.4f} seconds.")
        return cast(EarningsData, validated_model)
    else:
        logger.error(f"{log_prefix} Final validation failed for EarningsData model for symbol '{symbol}'.")
        return None 