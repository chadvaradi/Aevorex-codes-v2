...
from modules.financehub.backend.models import IndicatorHistory
...
async def calculate_indicators(
    ...
    try:
        if ohlcv_df is None or ohlcv_df.empty:
            logger.warning(f"{log_prefix} Empty OHLCV dataframe – skipping indicator calculation")
            return None

        history: IndicatorHistory | None = calculate_and_format_indicators(ohlcv_df, symbol)
        if history is None:
        ...
        logger.error(f"{log_prefix} Unexpected error during indicator calculation: {e}", exc_info=True)
        return None
...
def extract_latest_indicators(history: IndicatorHistory | None) -> dict[str, Any]:
    ...
    try:
        ...
        # --- Bollinger Bands ---
        if history.bbands and history.bbands.BBANDS_UPPER and history.bbands.BBANDS_MIDDLE and history.bbands.BBANDS_LOWER:
        ...
        logger.error(f"extract_latest_indicators(): error while extracting values – {e}", exc_info=True)
