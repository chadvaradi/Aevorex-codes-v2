...
def safe_get(df: pd.DataFrame | None, index: Any, column: str, default: Any = None, *, context: str = "") -> Any:
    ...
    try:
        ...
        logger.error(f"Unexpected error in safe_get for index '{index}', column '{column}': {e}", exc_info=True)
        return default
    ...
    for col in common_date_cols:
        if col in df.columns:
            try:
                ...
                df = df.set_index(col)
                if isinstance(df.index, pd.DatetimeIndex):
                    logger.debug(f"Set DatetimeIndex from column '{col}' for {function_name}.")
                    return df
            except Exception:
                continue
