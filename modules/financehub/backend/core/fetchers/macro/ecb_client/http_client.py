...
from .exceptions import ECBAPIError, ECBConnectionError, ECBTimeoutError, ECBRateLimitError
...
class ECBHTTPClient:
    ...
    async def download_ecb_sdmx(
        ...
        try:
            ...
            raise ECBTimeoutError(f"ECB API request timed out after {duration:.2f}s", duration) from e
            ...
            logger.error(f"HTTP error fetching ECB data: {e.response.status_code} - {error_text}")
