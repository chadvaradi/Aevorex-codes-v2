...
async def fetch_fed_yield_curve_historical() -> pd.DataFrame:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # Normalise 'Date' column name case-insensitively as well
            def _normalize_col(c: str) -> str:
                if c.strip().lower() == "date":
                    return "Date"
                return rename_map.get(c, lowercase_map.get(c.lower(), c))
            ...
            if "Date" not in present_cols:
                # Fallback: attempt to detect any date-like column
                for c in df.columns:
                    if "date" in c.lower():
                        df.rename(columns={c: "Date"}, inplace=True)
                        present_cols = ["Date", *[col for col in KEEP_COLS[1:] if col in df.columns]]
                        break
            df = df[present_cols]
