...
async def fetch_ecb_fx_rates(
    ...
    logger.info(f"Fetching ECB FX rates for {currency_pair} from {start_date} to {end_date}")
    ...
    try:
        ...
        if cache:
            cached_data = await cache.get(cache_key)
            if cached_data:
                logger.warning("Returning STALE ECB FX rates from cache due to fetch error")
