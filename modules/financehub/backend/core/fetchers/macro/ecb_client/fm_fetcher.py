                                ...
                                if obs_value and len(obs_value) > 0:
                                    ...
                                    if len(key_parts) > time_dim_index:
                                        ...
                                        if "attributes" in structure:
                                            time_values = None
                                            for attr in structure["attributes"].get("observation", []):
                                                if attr.get("id") == "TIME_PERIOD":
                                                    time_values = attr.get("values", [])
                                                    break
                                                    
                                            if time_values and time_index < len(time_values):
                                                ...
                                                logger.info(f"Retrieved Euribor {tenor}: {rate_value} for {obs_date}")
