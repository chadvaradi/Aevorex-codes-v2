...
async def fetch_yield_curve_rates(
    ...
    logger.info(f"Fetching yield curve rates for maturities {maturities} from {start_date} to {end_date}")
    ...
    try:
        ...
        if cache_service and result:
            cache_key = f"ecb_yield_curve_{start_date}_{end_date}_{'_'.join(maturities)}"
            try:
    ...
    if cache_service:
        ...
        try:
            ...
            if cached_rate is not None:
                logger.debug(f"Retrieved cached yield rate for {maturity}: {cached_rate}")
