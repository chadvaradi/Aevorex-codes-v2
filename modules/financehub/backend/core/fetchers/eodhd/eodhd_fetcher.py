...
from modules.financehub.backend.core.fetchers.common._base_helpers import (
    FETCH_FAILED_MARKER,
    generate_cache_key,
    get_api_key,
    make_api_request,
)
from modules.financehub.backend.core.fetchers.common._fetcher_constants import (
    EODHD_BASE_URL_EOD,
...
from modules.financehub.backend.models import stock as sm
...
class EODHDFetcher(BaseFetcher):
        ...
        **kwargs
        ...
        try:
            ...
            if response_data is None:
                await self.cache.set(cache_key, FETCH_FAILED_MARKER, ttl=FETCH_FAILURE_CACHE_TTL)
                return None
            ...
            EODHD_FETCHER_LOGGER.info(f"{log_prefix} Fetch successful. Shape: {df.shape}")
            return df

        except Exception as e:
            EODHD_FETCHER_LOGGER.critical(f"{log_prefix} Critical failure in OHLCV fetch: {e}", exc_info=True)
            return None
    
    async def fetch_quote(self, ticker: str, force_refresh: bool = False, **kwargs) -> dict[str, Any] | None:
        ...
        if not response_json or not isinstance(response_json, dict):
            EODHD_FETCHER_LOGGER.warning(f"{log_prefix} API call failed or returned invalid data. Caching failure.")
    ...
    async def fetch_fundamentals(self, ticker: str, force_refresh: bool = False, **kwargs) -> dict | None:
        ...
        if not force_refresh:
            ...
            if cached_data:
                EODHD_FETCHER_LOGGER.info(f"{log_prefix} Cache HIT.")
                if cached_data == FETCH_FAILED_MARKER:
                    EODHD_FETCHER_LOGGER.warning(f"{log_prefix} Cached failure detected. Returning None.")
                    return None
                return cached_data
        ...
        EODHD_FETCHER_LOGGER.info(f"{log_prefix} Successfully fetched fundamentals data.")
        await self.cache.set(cache_key, response_json, ttl=EODHD_FUNDAMENTALS_TTL)
        return response_json

    async def fetch_company_info(self, ticker: str, force_refresh: bool = False, **kwargs) -> dict | None:
        EODHD_FETCHER_LOGGER.info(f"[EODHD-CompanyInfo:{ticker}] Fetching company info (via fundamentals).")
    ...
    async def fetch_financial_statements(
        self, ticker: str, statement_type: str, frequency: str = 'Annual', force_refresh: bool = False, **kwargs
    ) -> pd.DataFrame | None:
        EODHD_FETCHER_LOGGER.info(f"[EODHD-Financials:{ticker}] Fetching {frequency} {statement_type} (via fundamentals).")
        
        fundamentals = await self.fetch_fundamentals(ticker, force_refresh)
        
        if (
            fundamentals and 
            "Financials" in fundamentals and
            statement_type in fundamentals["Financials"] and
            frequency in fundamentals["Financials"][statement_type]
        ):
        ...
        EODHD_FETCHER_LOGGER.warning(f"[EODHD-Financials:{ticker}] Could not find {frequency} {statement_type} in fundamentals data.")
        return None
    ...
    async def fetch_news(
        ...
        if not response_json or not isinstance(response_json, list):
            EODHD_FETCHER_LOGGER.warning(f"{log_prefix} API call failed or returned invalid data.")
            return []
        
        # Using the mapper from the yfinance_fetcher as it is generic
        news_items = [sm.NewsItem(**article) for article in response_json]
        await self.cache.set(cache_key, [item.model_dump() for item in news_items], ttl=3600)
