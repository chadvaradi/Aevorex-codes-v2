...
async def format_fundamental_data_for_prompt(symbol: str, company_overview: CompanyOverview | None) -> tuple[str, bool]:
    """
    ...
    if not company_overview:
        logger.warning(f"[{symbol}] {func_name}: Skipping, no CompanyOverview data provided.")
    ...
    try:
        ...
        if not data_found:
            logger.warning(f"[{symbol}] {func_name}: No valid fundamental data points found in CompanyOverview object.")
        ...
        logger.debug(f"[{symbol}] {func_name}: Successfully formatted fundamental data.")
