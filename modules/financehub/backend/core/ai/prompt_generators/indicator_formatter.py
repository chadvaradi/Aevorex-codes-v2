...
def _format_bb_middle(latest_indicators: dict[str, Any], last_close: float | None, **kwargs) -> str | None:
...
def _format_bb_bands(latest_indicators: dict[str, Any], last_close: float | None, **kwargs) -> str | None:
...
async def format_indicator_data_for_prompt(
    ...
    try:
        ...
        for indicator_key, formatter_func in FORMATTER_REGISTRY.items():
            logger.debug(f"[{symbol}] {func_name}: Processing indicator '{indicator_key}'..")
            try:
                ...
                logger.error(f"[{symbol}] {func_name}: Error processing indicator '{indicator_key}': {e}", exc_info=True)
                summary_points.append(f"Hiba a(z) '{indicator_key}' indikátor feldolgozása közben.")
        
        if not summary_points:
            logger.warning(f"[{symbol}] {func_name}: No indicator data points could be summarized.")
            return "Nem sikerült a technikai indikátorokból összefoglalót készíteni.", False
