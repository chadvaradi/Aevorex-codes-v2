...
async def format_earnings_data_for_prompt(symbol: str, earnings_data: EarningsData | None) -> str:
    """
    ...
    try:
        ...
        if earnings_data.history and not earnings_data.history.empty:
            ...
            for _, row in earnings_data.history.head(4).iterrows():
                report_date = safe_format_value(symbol, "reportDate", row.get("reportDate"))
                eps_actual = safe_format_value(symbol, "epsActual", row.get("epsActual"))
                eps_estimate = safe_format_value(symbol, "epsEstimate", row.get("epsEstimate"))
                
                # Only include if we have some data
                if eps_actual != "N/A" or eps_estimate != "N/A":
                    history_points.append(f"- Dátum: {report_date}, Jelentett EPS: {eps_actual}, Várt EPS: {eps_estimate}")
            
            if history_points:
                summary_points.append("== Elmúlt Negyedéves Jelentések (EPS) ==\n" + "\n".join(history_points))
                data_found = True
        ...
        if earnings_data.trend and not earnings_data.trend.empty:
            ...
            for _, row in earnings_data.trend.head(4).iterrows():
                ...
                revenue_estimate = safe_format_value(symbol, "revenueEstimate", row.get("revenueEstimate"), ",")
                earnings_estimate = safe_format_value(symbol, "earningsEstimate", row.get("earningsEstimate"), ",")
                
                if revenue_estimate != "N/A" or earnings_estimate != "N/A":
                    trend_points.append(f"- Időszak: {period}, Várt árbevétel: {revenue_estimate}, Várt nyereség: {earnings_estimate}")
            
            if trend_points:
                summary_points.append("== Elemzői Várakozások (Negyedéves) ==\n" + "\n".join(trend_points))
                data_found = True
        ...
        if earnings_data.annual and not earnings_data.annual.empty:
            ...
            for _, row in earnings_data.annual.head(2).iterrows():
                ...
                if eps != "N/A":
                    annual_points.append(f"- Év (jelentés dátuma): {report_date}, Teljes éves EPS: {eps}")
            
            if annual_points:
                summary_points.append("== Éves Jelentések (EPS) ==\n" + "\n".join(annual_points))
                data_found = True

        if not data_found:
            logger.warning(f"[{symbol}] {func_name}: No valid data points found in EarningsData object.")
