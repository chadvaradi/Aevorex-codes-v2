...
async def format_financials_data_for_prompt(symbol: str, financials_data: FinancialsData | None) -> str:
    """
    ...
    try:
        ...
        for section_title, statement_df in STATEMENT_MAPPING.items():
            ...
            if not statement_df.columns.empty:
                ...
                logger.debug(f"[{symbol}] {func_name}: Processing '{section_title}' for latest year '{latest_year}'.")
                ...
                for index, value in statement_df[latest_year].head(10).items():
                    ...
                    if pd.notna(value) and value != 0:
                        formatted_value = safe_format_value(symbol, str(index), value, ",")
                        # Clean up index name (e.g., "totalRevenue" -> "Total Revenue")
                        label = " ".join(word.capitalize() for word in re.sub(r"([A-Z])", r" \1", str(index)).split())
        ...
        if not data_found:
            logger.warning(f"[{symbol}] {func_name}: No valid data points found in any financial statement.")
