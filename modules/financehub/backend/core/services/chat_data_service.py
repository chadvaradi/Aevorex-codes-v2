    ...
    async def get_stock_data_for_chat(
        ...
        try:
            ...
            tasks = {
                ...
                "technicals": self.technical_service.get_technical_analysis(symbol, client, cache, force_refresh),
                "news": self.news_service.get_news_data(symbol, client, cache, limit=20, force_refresh=force_refresh),
                "chart": self.chart_service.get_chart_data(symbol, client, cache, period="1y", interval="1d", force_refresh=force_refresh),
            }
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    logger.error(f"{log_prefix} Task '{task_name}' failed: {result}", exc_info=result)
            ...
            if not final_response_model:
                logger.error(f"{log_prefix} Orchestrator failed to build response model.")
                return None

            logger.info(f"{log_prefix} Successfully fetched and orchestrated data for chat context.")
            return final_response_model

        except Exception as e:
            logger.error(f"{log_prefix} An unexpected error occurred during chat data fetching: {e}", exc_info=True)
            return None
