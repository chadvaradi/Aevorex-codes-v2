...
def parse_fmp_quote_response(api_response_data: Any, symbol: str) -> dict[str, Any] | None:
    log_prefix = f"{MODULE_PREFIX} [ParseFMP:{symbol}]"
    if not isinstance(api_response_data, list) or not api_response_data:
        logger.warning(f"{log_prefix} Unexpected API response format (expected non-empty list).")
        return None
    ...
    try:
        ...
        return None
    return {"symbol": symbol, "price": round(current_price, 4), "change": round(change, 4), "change_percent": round(change_percent, 2)}

def parse_alpha_vantage_quote_response(api_response_data: Any, symbol: str) -> dict[str, Any] | None:
    ...
    if symbol_api != symbol:
        logger.warning(f"{log_prefix} Mismatched symbol. Expected '{symbol}', got '{symbol_api}'.")
    if current_price_str is None or change_str is None or change_percent_str is None:
        return None
    try:
        current_price = float(current_price_str)
...
def parse_eodhd_realtime_response(api_response_data: Any, full_symbol: str) -> dict[str, Any] | None:
    ...
    if isinstance(api_response_data, dict):
        try:
            ...
            return {"symbol": full_symbol, "price": round(current_price, 4), "change": round(change, 4), "change_percent": round(change_percent, 2)}
...
def _fetch_yf_quote_sync(symbol: str) -> dict[str, Any] | None:
    ...
    """
    if yf is None:
        logger.error(f"{MODULE_PREFIX} yfinance package missing – cannot use YF provider.")
        return None
    try:
        t = yf.Ticker(symbol)
...
def normalize_symbol_for_provider(symbol: str, provider: str) -> str:
    ...
    if provider == 'EODHD':
        ...
        if '.' not in symbol and not symbol.startswith('^') and '=' not in symbol and '-' not in symbol:
            return f"{symbol}.US"
...
async def fetch_single_ticker_quote(
    ...
    try:
        ...
        if parser:
            ...
            if parsed is None:
                logger.warning(f"{log_prefix} Failed to parse response from provider – attempting YF fallback…")
        ...
        logger.error(f"{log_prefix} An unexpected error occurred. Error: {exc}. Attempting YF fallback…")
        return await asyncio.to_thread(_fetch_yf_quote_sync, symbol) 