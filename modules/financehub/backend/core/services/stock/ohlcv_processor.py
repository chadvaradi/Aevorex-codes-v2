...
class OHLCVProcessor:
    ...
    """
    async def process_ohlcv_data(self, ohlcv_data: Any, request_id: str) -> list[dict[str, Any]] | None:
        """Process raw OHLCV data into chart-ready format."""
        try:
            if hasattr(ohlcv_data, 'to_dict'):  # DataFrame
                logger.debug(f"[{request_id}] Mapping yfinance OHLCV DataFrame to chart list.")
                return mappers.map_yfinance_ohlcv_df_to_chart_list(ohlcv_data, request_id)
