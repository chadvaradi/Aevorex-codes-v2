...
class TechnicalProcessor:
    ...
    def calculate_all_indicators(self, ohlcv_df: pd.DataFrame) -> pd.DataFrame | None:
        ...
        try:
            ...
            logger.info(f"Successfully calculated {len(custom_strategy.ta)} types of indicators.")
    ...
    def extract_latest_values(self, indicators_df: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            indicator_columns = [col for col in indicators_df.columns if col.upper() not in ['OPEN', 'HIGH', 'LOW', 'CLOSE', 'VOLUME', 'TICKER']]
            ...
            logger.error(f"Error extracting latest indicator values: {e}", exc_info=True)
            return {}
    ...
    def format_indicator_summary(self, latest_indicators: dict[str, float | None]) -> dict[str, Any]:
