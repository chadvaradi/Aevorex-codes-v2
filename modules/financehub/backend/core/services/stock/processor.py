...
from modules.financehub.backend.models.stock import CompanyOverview, NewsItem, LatestOHLCV
from modules.financehub.backend.utils.logger_config import get_logger
from modules.financehub.backend.utils.helpers import parse_optional_int
...
class StockDataProcessor:
    ...
    def process_company_info(self, ticker_info: dict[str, Any], symbol: str, request_id: str = None) -> CompanyOverview | None:
        ...
        try:
            company_overview = CompanyOverview(
                symbol=symbol,
                company_name=ticker_info.get('longName') or ticker_info.get('shortName', symbol),
            ...
            logger.error(f"[{request_id}] Error processing company info for {symbol}: {e}")
            return None
    ...
    def process_ohlcv_dataframe(
        ...
        try:
            ...
            latest_ohlcv = LatestOHLCV(
                t=int(last_row.name.timestamp()) if hasattr(last_row.name, 'timestamp') else int(datetime.now().timestamp()),
    ...
    def validate_and_clean_data(
        ...
        try:
            ...
            for key, value in data.items():
                ...
                if value is None:
                    if key in ['current_price', 'previous_close', 'change', 'change_percent']:
