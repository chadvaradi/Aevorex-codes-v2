...
from modules.financehub.backend.core.helpers import (
    parse_optional_float, parse_optional_int
)
...
class DataProcessor:
    ...
    def process_company_info(
        ...
        try:
            ...
            processed_data = {
                "symbol": symbol,
                "company_name": raw_data.get("longName") or raw_data.get("shortName") or symbol,
                "sector": raw_data.get("sector", self.default_na_value),
                "industry": raw_data.get("industry", self.default_na_value),
                "description": raw_data.get("longBusinessSummary", self.default_na_value),
            ...
            logger.error(f"[{request_id}] Failed to process company info for {symbol}: {e}")
            return None
    ...
    def process_ohlcv_data(
        ...
        try:
            ...
            if missing_cols:
                logger.error(f"[{request_id}] Missing OHLCV columns for {symbol}: {missing_cols}")
                return None
            ...
            if df.empty:
                logger.warning(f"[{request_id}] No valid OHLCV data after cleaning for {symbol}")
                return None
            
            # Sort by date
            df = df.sort_index()
            
            logger.debug(f"[{request_id}] Processed {len(df)} OHLCV records for {symbol}")
            return df
            
        except Exception as e:
            logger.error(f"[{request_id}] Failed to process OHLCV data for {symbol}: {e}")
            return None
    ...
    def process_news_data(
        ...
        for item in raw_data:
            try:
                ...
                logger.debug(f"[{request_id}] Failed to process news item for {symbol}: {e}")
                continue
        
        logger.debug(f"[{request_id}] Processed {len(processed_news)} news items for {symbol}")
    ...
    def extract_latest_ohlcv(
        ...
        try:
            ...
            return LatestOHLCV(
                symbol=symbol,
                date=latest_row.name.isoformat() if hasattr(latest_row.name, 'isoformat') else str(latest_row.name),
            ...
            logger.error(f"[{request_id}] Failed to extract latest OHLCV for {symbol}: {e}")
            return None
