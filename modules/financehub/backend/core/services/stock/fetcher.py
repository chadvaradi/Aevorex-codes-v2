...
class StockDataFetcher:
    ...
    async def fetch_company_info(
        ...
        try:
            ...
            logger.error(f"[{request_id}] Error fetching company info for {symbol}: {e}")
            return None
    ...
    async def fetch_news_data(
        ...
        try:
            news_items = await self.yfinance_fetcher.fetch_news(
                ticker=symbol
            )
            
            logger.debug(f"[{request_id}] Fetched {len(news_items)} news items for {symbol}")
    ...
    async def fetch_parallel_data(
        ...
        tasks = {
            'company_info': self.fetch_company_info(symbol, request_id)
        }
        ...
        try:
            ...
            for i, (key, _) in enumerate(tasks.items()):
                ...
                if isinstance(result, Exception):
                    logger.error(f"Task {key} failed for {symbol} with error: {result}", exc_info=True)
