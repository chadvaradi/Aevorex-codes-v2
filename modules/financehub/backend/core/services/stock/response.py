...
from modules.financehub.backend.models.stock import (
    CompanyOverview, NewsItem, LatestOHLCV
)
...
class StockResponseBuilder:
    ...
    def build_chart_response(self, symbol: str, ohlcv_df: pd.DataFrame, latest_ohlcv: LatestOHLCV | None, request_id: str = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not ohlcv_df.empty:
                ...
                for idx, row in ohlcv_df.iterrows():
                    ...
                    chart_row = {
                        "timestamp": int(idx.timestamp()) if hasattr(idx, "timestamp") else str(idx),
            ...
            logger.debug(f"[{request_id}] Chart response built for {symbol} with {len(chart_data)} points")
            return response
        except Exception as e:
            logger.error(f"[{request_id}] Error building chart response for {symbol}: {e}")
    ...
    def build_fundamentals_response(
        ...
        try:
            ...
            def _clean(value):
                ...
                if value is None:
                    return None
                if isinstance(value, (float, int)) and (math.isnan(value) if isinstance(value, float) else False):
                    return None
                if pd.isna(value):
                    return None
                return value
            ...
            financial_metrics = {
                ...
                "price_to_sales": _clean(raw_ticker_info.get("priceToSalesTrailing12Months")),
            ...
            logger.error(f"[{request_id}] Error building fundamentals response for {symbol}: {e}")
            return self._build_error_response(symbol, "fundamentals_data_error", request_id)
        ...
        try:
            ...
            for item in news_items:
                news_data.append({
                    ...
                    "published_date": item.published_at.isoformat() if item.published_at else None,
                    "source": item.publisher,
                    "sentiment": getattr(item, 'overall_sentiment_label', 'neutral')
                })
            
            # Calculate sentiment if not provided
            if not sentiment_summary:
                pos = sum(1 for item in news_items if getattr(item, 'overall_sentiment_label', '') == 'positive')
                neg = sum(1 for item in news_items if getattr(item, 'overall_sentiment_label', '') == 'negative')
                neu = len(news_items) - pos - neg
                
                overall = "positive" if pos > max(neg, neu) else "negative" if neg > max(pos, neu) else "neutral"
                sentiment_summary = {
                    "overall": overall,
                    "positive": pos,
                    "negative": neg,
            ...
            logger.debug(f"[{request_id}] News response built for {symbol} with {len(news_items)} items")
            return response
            
        except Exception as e:
            logger.error(f"[{request_id}] Error building news response for {symbol}: {e}")
