...
class CacheManager:
        ...
        try:
            ...
            if cached_data:
                ...
                if isinstance(cached_data, str):
                    ...
                    logger.warning(f"[{request_id}] Invalid cached data type: {type(cached_data)}")
                    return None
    ...
    async def cache_final_response(
        ...
        try:
            ...
            logger.error(f"[{request_id}] Failed to cache response for key {cache_key}: {e}")
            return False
        ...
        **kwargs
        ...
        try:
            async with lock:
                ...
                cached_result = await self.check_aggregate_cache(cache_key, request_id, cache)
                if cached_result:
                    logger.debug(f"[{request_id}] Found cached result after lock acquisition")
                ...
                if result:
                    await self.cache_final_response(cache_key, result, request_id, cache)
            ...
            logger.error(f"[{request_id}] Error in locked operation for {lock_key}: {e}")
            raise
    ...
    async def invalidate_cache(
        ...
        try:
            ...
            if keys:
                ...
                logger.info(f"[{request_id}] Invalidated {len(keys)} cache entries for {symbol}")
            else:
                logger.debug(f"[{request_id}] No cache entries to invalidate for {symbol}")
