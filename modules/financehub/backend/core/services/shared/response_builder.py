...
from modules.financehub.backend.models.stock import FinBotStockResponse, NewsItem, CompanyOverview, FinancialsData, LatestOHLCV, IndicatorHistory
from modules.financehub.backend.core.services.stock.fundamentals_processors import FundamentalsProcessor
from modules.financehub.backend.core.services.shared.response_helpers import process_ohlcv_dataframe, process_technical_indicators
...
async def build_stock_response_from_parallel_data(
    ...
    try:
        if isinstance(fundamentals_raw, dict) and fundamentals_raw:
            ...
            company_overview = fundamentals_processor.process_company_info(fundamentals_raw, symbol)
            financials_data = fundamentals_processor.process_financials_data(fundamentals_raw)
            earnings_data = fundamentals_processor.extract_market_metrics(fundamentals_raw)
        else:
            logger.debug("%s fundamentals_raw missing or invalid – treated as empty", log_prefix)
    except Exception as exc:
        logger.warning("%s fundamentals processing failed: %s", log_prefix, exc, exc_info=True)
    ...
    # ---------------------------------------------------------------------
    latest_indicators_obj = process_technical_indicators(technical_indicators, log_prefix)
    ...
    logger.info("%s FinBotStockResponse built. history_len=%d, news=%d", log_prefix, len(history_entries), len(processed_news_items))
    return response
...
def build_finbot_stock_response(
    ...
    # Process News
    from modules.financehub.backend.models.stock.response_models import NewsData
    ...
    indicator_history = IndicatorHistory(**technical_indicators) if technical_indicators else IndicatorHistory()
