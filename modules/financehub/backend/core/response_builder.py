...
from modules.financehub.backend.models import (
    FinBotStockResponse, IndicatorHistory, CompanyOverview, NewsItem,
    CompanyPriceHistoryEntry, LatestOHLCV, FinancialsData, EarningsData
)
...
def build_stock_response(
    ...
    try:
        ...
        history_ohlcv = format_ohlcv_history(ohlcv_df, symbol) if ohlcv_df is not None else []
        
        # Build latest OHLCV
        latest_ohlcv = build_latest_ohlcv(ohlcv_df, symbol) if ohlcv_df is not None else None
...
def format_ohlcv_history(ohlcv_df: pd.DataFrame, symbol: str) -> list[CompanyPriceHistoryEntry]:
