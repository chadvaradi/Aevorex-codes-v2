...
# from modules.financehub.backend.core.chat.context_manager import InMemoryHistoryManager, AbstractHistoryManager # Ha még használatban van
# from modules.financehub.backend.core.chat.chat_service import ChatService
...
async def lifespan_manager(app: FastAPI):
    ...
    resources_initialized = {"http_client": False, "cache_service": False, "orchestrator": False, "history_manager": False}
    ...
    try:
        ...
        if resources_initialized["http_client"] and resources_initialized["cache_service"]:
            ...
            try:
                ...
                _orchestrator_instance = StockOrchestrator(cache=_cache_service_instance)
                app.state.orchestrator = _orchestrator_instance
                logger.info("[Lifespan] Global StockOrchestrator initialized and assigned to app.state.orchestrator.")
                resources_initialized["orchestrator"] = True
            except Exception as e:
                logger.critical(f"[Lifespan] CRITICAL FAILURE initializing StockOrchestrator: {e}", exc_info=True)
        ...
        if all([resources_initialized["http_client"], resources_initialized["cache_service"], resources_initialized["orchestrator"]]):
...
async def get_http_client(request: Request) -> httpx.AsyncClient:
    ...
    try:
        ...
        fallback_client = httpx.AsyncClient(timeout=timeout, limits=limits, headers=headers, http2=True, follow_redirects=True)
        # Save to globals so subsequent calls reuse the same client
        _http_client_instance = fallback_client
        logger.warning("[Dependency Fallback] Created on-the-fly httpx.AsyncClient fallback instance.")
...
async def get_cache_service(request: Request) -> CacheService:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Cache service unavailable and fallback failed")
...
async def get_orchestrator(request: Request) -> StockOrchestrator:
    ...
    if hasattr(request.app.state, 'orchestrator') and request.app.state.orchestrator is not None:
        ...
        try:
            ...
            logger.warning("[Dependency Fallback] StockOrchestrator lazily instantiated on demand.")
