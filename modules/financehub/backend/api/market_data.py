...
async def get_market_news(
    ...
    try:
        ...
        if cached_data_raw is None:
            logger.info(f"{LOG_PREFIX_ENDPOINT} Cache MISS for key: '{cache_key}'. Fetching from NewsAPI.org")

            api_key = os.getenv("FINBOT_API_KEYS__NEWSAPI") or os.getenv("NEWSAPI_API_KEY")
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                if api_key:
                    ...
                    try:
                        ...
                        raise HTTPException(status_code=502, detail=f"NewsAPI error: {http_err}") from http_err

                    payload = resp.json()
                    if payload.get("status") != "ok":
                        raise HTTPException(status_code=502, detail="Unexpected response from NewsAPI.org")
                    ...
                    if payload.get("Type") != 100:  # Success code for CryptoCompare
                        raise HTTPException(status_code=502, detail="Unexpected response from CryptoCompare news API")
                ...
                for art in articles:
                    news_data.append({
                        "headline": art.get("title") or art.get("title") or art.get("Headline"),
                        "summary": art.get("description") or art.get("Body"),
                        "url": art.get("url") or art.get("Url"),
                        "source": art.get("source", {}).get("name") if isinstance(art.get("source"), dict) else art.get("source") or source_name,
                        "timestamp": art.get("publishedAt") or art.get("published_on"),
            ...
            logger.info(f"{LOG_PREFIX_ENDPOINT} Successfully fetched {len(news_data)} news items from NewsAPI")
            return news_data
...
try:
    ...
    logger.warning("yfinance not installed – /indices endpoint will be unavailable: %s", imp_err)
    yf = None  # type: ignore
...
async def get_market_indices(
    limit: Annotated[int | None, Query(description="Max number of indices to return", ge=1, le=10)] = None,
):
    if yf is None:
        raise HTTPException(status_code=503, detail="yfinance dependency unavailable on server")
    ...
    if not indices_data:
        raise HTTPException(status_code=502, detail="Failed to fetch index data from provider (yfinance)")
