...
from modules.financehub.backend.core.fetchers.macro.ecb_client.euribor_client import get_latest_euribor_rates
...
async def get_fixing_rates(
    date_filter: date = Query(default_factory=date.today, description="Date for fixing rates"),
    force_live: bool = Query(True, description="Always fetch fresh live data by default"),
    ...
    try:
        ...
        logger.info(f"Fetching live fixing rates for all tenors from {fetch_start} to {date_filter}")

        # Always fetch fresh ECB ESTR (overnight) data
        try:
            estr_data = await fetch_ecb_estr_rate(None, fetch_start, date_filter)  # Pass None for cache
            logger.info(f"ECB ESTR data fetched: {len(estr_data) if estr_data else 0} dates")
        ...
        try:
            euribor_rates = await get_latest_euribor_rates()  # Web scraping from euribor-rates.eu
            logger.info(f"Official Euribor rates fetched: {len(euribor_rates) if euribor_rates else 0} tenors")
        except Exception as e:
        ...
        try:
            bubor_data = await fetch_bubor_curve(fetch_start, date_filter, None)  # Pass None for cache
            logger.info(f"BUBOR data fetched: {len(bubor_data) if bubor_data else 0} dates")
        except Exception as e:
            ...
            if estr_data:
                ...
                if dates:
                    ...
                    if "ESTR_Rate" in estr_data[latest_date]:
                        ...
                        logger.info(f"Retrieved ECB ESTR: {estr_rate} for {latest_date}")
                ...
                if dates:
                    ...
                    for bubor_tenor, our_tenor in tenor_mapping.items():
                        if bubor_tenor in latest_bubor:
                            ...
                            logger.info(f"Retrieved BUBOR {our_tenor}: {latest_bubor[bubor_tenor]}")

            # Combine ECB ESTR + Official Euribor (NO YIELD CURVE)
            combined_rates = {"ON": estr_rate}  # ECB ESTR for overnight
            combined_rates.update(euribor_rates)  # Add official Euribor rates for all other tenors
        ...
        if not ecb_final and not bubor_final:
            raise HTTPException(status_code=503, detail="All fixing rate providers unavailable – please retry later.")
        ...
        return {
            ...
            "metadata": {
                ...
                "reference_dates": {
                    ...
                    "euribor": (date_filter - timedelta(days=1)).isoformat(),  # Euribor T-1 delay
...
async def health_check(
    ...
    try:
        test_estr = await fetch_ecb_estr_rate(None, current_time - timedelta(days=7), current_time)
    ...
    try:
        test_bubor = await fetch_bubor_curve(current_time - timedelta(days=7), current_time, None)
...
@router.get("/validation", summary="Cross-reference validation against official sources")
async def validate_against_official_sources(
    date_filter: date = Query(default_factory=date.today, description="Date to validate"),
    cache: CacheService = Depends(get_cache_service),
) -> Dict[str, Any]:
    """
