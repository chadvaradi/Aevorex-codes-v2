...
from modules.financehub.backend.core.fetchers.macro.fred_client import fetch_fred_series_matrix
...
async def get_fed_policy(
    request: Request,
    series: List[str] = Query(["EFFR"], description="FRED series IDs, e.g., EFFR, IORB"),
    ...
    try:
        _t0 = perf_counter()
        matrix = await fetch_fred_series_matrix(http, api_key, series, start_date, end_date)
