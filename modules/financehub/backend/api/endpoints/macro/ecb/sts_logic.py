...
from modules.financehub.backend.core.fetchers.macro.ecb_client import fetch_ecb_sts_data
...
async def get_sts_response(
    ...
    try:
        ...
        metadata = {
            ...
            "frequency": "monthly",
            "indicators": sorted({k for rec in sts_data.values() for k in rec}) if sts_data else [],
        }
...
def _empty_payload(message: str = "", start_date: Optional[date] | None = None, end_date: Optional[date] | None = None) -> JSONResponse:
    return JSONResponse(status_code=200, content={
        "status": "success",
        "metadata": {
