...
def map_short_end_tenors(policy_rates: Dict[str, Dict[str, float]]) -> Dict[str, Dict[str, Any]]:
    ...
    for date_str, rates in policy_rates.items():
        ...
        for tenor in SHORT_TENORS:
            if tenor == "on":
                ...
                row[tenor] = rates.get("Euro Short-Term Rate") or rates.get("Deposit Facility Rate", df_rate)
            elif tenor in ("1w", "2w"):
