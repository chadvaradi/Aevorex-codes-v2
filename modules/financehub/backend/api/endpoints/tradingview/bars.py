...
from modules.financehub.backend.core.fetchers.macro.ecb_client.estr_fetcher import fetch_ecb_estr_rate
from modules.financehub.backend.core.fetchers.macro.ecb_client.yc_fetcher import fetch_yield_curve_rates, fetch_single_yield_rate
from modules.financehub.backend.core.fetchers.macro.ecb_client.policy_rates_fetcher import fetch_policy_rates, fetch_single_policy_rate
from modules.financehub.backend.core.fetchers.macro.ecb_client.fx_rates_fetcher import fetch_fx_rates, fetch_single_fx_rate
from modules.financehub.backend.core.fetchers.macro.ecb_client.euribor_client import fetch_official_euribor_rates
from modules.financehub.backend.core.fetchers.macro.ecb_client.bubor_client import fetch_bubor_curve
...
async def get_bars(
    request: Request,
    symbol: str = Query(..., description="Symbol (e.g., EURIBOR_3M, BUBOR_3M, YC_SR_10Y)"),
    period: str = Query("1D", description="Time period (1D only supported)"),
    from_date: str = Query(..., alias="from", description="Start date (YYYY-MM-DD or Unix timestamp)"),
    to_date: str = Query(..., alias="to", description="End date (YYYY-MM-DD or Unix timestamp)"),
    ...
    """
    logger.info(f"Fetching TradingView bars for {symbol}, period={period}, from={from_date}, to={to_date}")
    ...
    try:
        ...
        return {
            "s": "no_data",
            "nextTime": int(datetime.now().timestamp())
        }
    ...
    try:
        ...
        for date_str, data in sorted(estr_data.items()):
            if "ESTR_Rate" in data:
                times.append(unix_timestamp(datetime.strptime(date_str, "%Y-%m-%d").date()))
...
async def fetch_euribor_hsta_data(symbol: str, start_date: date, end_date: date) -> Dict[str, List]:
...
async def fetch_euribor_demo_data(symbol: str, start_date: date, end_date: date) -> Dict[str, List]:
...
async def fetch_bubor_data(symbol: str, start_date: date, end_date: date) -> Dict[str, List]:
    ...
    try:
        ...
        if not bubor_key:
            raise HTTPException(status_code=404, detail=f"BUBOR tenor {tenor} not supported")
...
async def fetch_yield_curve_data(symbol: str, start_date: date, end_date: date) -> Dict[str, List]:
...
async def fetch_policy_rate_data(symbol: str, start_date: date, end_date: date) -> Dict[str, List]:
...
async def fetch_fx_rate_data(symbol: str, start_date: date, end_date: date) -> Dict[str, List]:
    ...
    try:
        ...
        # Validate currency
        valid_currencies = ["USD", "GBP", "HUF", "CHF", "JPY", "CAD", "AUD", "SEK", "NOK", "DKK", "PLN", "CZK"]
