...
def _compute_factors_from_closes(closes: list[float], window: int = 20) -> dict[str, float]:
...
async def get_factors(
    ...
    return FactorResponse(ticker=ticker.upper(), as_of=as_of, window_days=window, factors=factors)
...
async def get_signals(
    ...
    return SignalResponse(
        ...
        as_of=as_of,
        signal=signal,
        rationale={"factors": factors, "thresholds": {"momentum_20d": mom_th, "vol_20d": vol_th}},
    )
