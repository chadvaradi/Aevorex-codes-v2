...
router = APIRouter(
    prefix="/crypto",
    tags=["Cryptocurrency Data"]
)

@router.get(
    "/symbols",
    summary="Get Supported Crypto Symbols",
    status_code=status.HTTP_200_OK
)
async def get_crypto_symbols(cache: Annotated[CacheService, Depends(get_cache_service)]):
...
@router.get(
    "/{symbol}",
    summary="Get Crypto Rate for a Symbol",
    status_code=status.HTTP_200_OK
)
async def get_crypto_rate(
    ...
    try:
        ...
        if coin_id is None:
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.get("https://api.coingecko.com/api/v3/coins/list?include_platform=false")
                if resp.status_code != 200:
                    raise HTTPException(status_code=502, detail="CoinGecko lookup failed")
                coins = resp.json()
                for c in coins:
        ...
        if coin_id is None:
            raise HTTPException(status_code=404, detail="Symbol not supported by CoinGecko")
        ...
        if cache:
            await cache.set(cache_key_rate, rate_value, ttl=15)  # 15s TTL for near-real-time quotes
