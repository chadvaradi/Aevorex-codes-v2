...
# from modules.financehub.backend.core.chat.model_selector import MODEL_CATALOGUE
...
try:
    ...
    _logger.warning("MODEL_CATALOGUE missing – attempting runtime fetch from OpenRouter (%s)", import_err)

    OPENROUTER_KEY = os.getenv("FINBOT_API_KEYS__OPENROUTER") or os.getenv("OPENROUTER_API_KEY")
    ...
    if OPENROUTER_KEY:
        try:
            ...
            for m in data:
                ...
                tags_raw = m.get("openrouter", {}).get("tags") if isinstance(m.get("openrouter"), dict) else None
                ux_hint_val = "n/a"
            ...
            _logger.info("Fetched %s models from OpenRouter runtime", len(MODEL_CATALOGUE))
...
async def list_models(catalogue: Annotated[list[ModelMeta], Depends(get_catalogue_dependency)]):
...
async def list_models_alias(catalogue: Annotated[list[ModelMeta], Depends(get_catalogue_dependency)]):
