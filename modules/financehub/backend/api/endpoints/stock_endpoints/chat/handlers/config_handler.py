...
async def handle_toggle_deep_flag(payload: DeepToggleRequest, cache: CacheService):
    ...
    await cache.set(session_key, str(payload.needs_deep), ttl=600)  # Cache for 10 minutes
    logger.info(f"Set deep analysis flag for chat {payload.chat_id} to {payload.needs_deep}") 