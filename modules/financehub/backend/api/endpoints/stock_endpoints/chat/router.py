from fastapi import APIRouter, Depends, Request, Body, Path, Query, HTTPException, status
import json
...
from modules.financehub.backend.core.chat.prompt_builder import prompt_preprocessor
from modules.financehub.backend.models.chat import ChatResponse, ChatRequest
...
from modules.financehub.backend.core.ai.token_utils import estimate_tokens
from modules.financehub.backend.config import settings
from modules.financehub.backend.core.security.sse_token import verify_sse_token, mint_sse_token
...
async def stream_chat_response(
    ...
    message: str | None = Query(None, description="Optional user message to steer the stream (passes preprocessor)"),
    ...
    if settings.ENVIRONMENT.NODE_ENV == "production":
        if not auth or not verify_sse_token(auth):
            return JSONResponse({"status": "unauthorized", "code": 401, "message": "Invalid SSE token"}, status_code=401)
    hdr_plan = request.headers.get("x-plan")
    if hdr_plan:
        plan = hdr_plan
    if settings.ENVIRONMENT.NODE_ENV == "production" and plan == "free":
        return JSONResponse({"status": "forbidden", "code": 403, "message": "Upgrade required for streaming AI summary."}, status_code=403)
    ...
    async def event_generator():
        ...
        try:
            ...
            if message:
                ...
                try:
                    processed = await prompt_preprocessor(ticker=ticker, chat_req=ChatRequest(message=trimmed), request=request)
                ...
                async for token in ai_service.stream_chat(
                    ...
                    plan=plan,
                    context_messages=[],
                    query_type=(processed.get("query_type") if isinstance(processed, dict) else "summary"),
                    data_block=(processed.get("data_block") if isinstance(processed, dict) else None),
                ):
                ...
                try:
                    processed = await prompt_preprocessor(ticker=ticker, chat_req=ChatRequest(message=f"Provide a brief market summary for {ticker}"), request=request)
                except Exception:
...
async def stream_deep_analysis(
    ...
    if settings.ENVIRONMENT.NODE_ENV != "production":
        ...
        if plan not in ("pro", "team", "enterprise"):
            return JSONResponse({"status": "payment_required", "code": 402, "message": "Upgrade to Pro for deep analysis."}, status_code=402)
