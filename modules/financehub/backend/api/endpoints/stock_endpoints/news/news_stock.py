    ...
    try:
        ...
        for n in news_items_raw[:limit]:
            ...
            news_items.append({
                ...
                "published_date": (
                    _get(n, "published_at", "published_date").strftime("%Y-%m-%d %H:%M:%S")
                    if isinstance(_get(n, "published_at", "published_date"), datetime) else _get(n, "published_at", "published_date")
                ),
                "source": _get(n, "publisher", "source"),
                "sentiment": _get(n, "overall_sentiment_label", "sentiment", default="neutral"),
            })

        # Simple sentiment summary
        pos = sum(1 for n in news_items_raw if getattr(n, "overall_sentiment_label", "neutral") == "positive")
        neg = sum(1 for n in news_items_raw if getattr(n, "overall_sentiment_label", "neutral") == "negative")
