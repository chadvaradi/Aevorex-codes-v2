...
async def set_model(request: dict | ChatModelRequest | None = Body(None), cache: CacheService = Depends(get_cache_service)):
...
async def toggle_deep(request: dict | DeepToggleRequest | None = Body(None), cache: CacheService = Depends(get_cache_service)):
...
async def set_language(request: dict | None = Body(default=None), cache: CacheService = Depends(get_cache_service)):
