...
class PathSettings(BaseModel):
    ...
    def _resolve_and_verify_paths(self) -> 'PathSettings':
        ...
        self.LOG_DIR = self._ensure_directory(self.LOG_DIR, self.PROJECT_ROOT / "logs", "LOG_DIR", True)
        
        return self

    def _ensure_directory(self, path: Path | None, default_path: Path, name: str, check_writable: bool = False) -> Path:
        """Helper to create and verify a directory."""
        dir_to_check = path or default_path
        
        try:
            dir_to_check.mkdir(parents=True, exist_ok=True)
            if check_writable and not (os.access(dir_to_check, os.W_OK) and os.access(dir_to_check, os.R_OK)):
                logger.warning(f"{name} directory is not readable/writable: {dir_to_check}")
        except Exception as e:
            logger.error(f"Failed to create or access {name} directory at {dir_to_check}: {e}")
            raise
