...
class FileProcessingSettings(BaseModel):
    ...
    def _parse_string_list_to_lower(cls, values):
        if 'ALLOWED_MIME_TYPES' in values and values['ALLOWED_MIME_TYPES']:
            values['ALLOWED_MIME_TYPES'] = _parse_env_list_str_utility(values['ALLOWED_MIME_TYPES'], 'ALLOWED_MIME_TYPES', lowercase_items=True)
        if 'ALLOWED_EXTENSIONS' in values and values['ALLOWED_EXTENSIONS']:
            values['ALLOWED_EXTENSIONS'] = _parse_env_list_str_utility(values['ALLOWED_EXTENSIONS'], 'ALLOWED_EXTENSIONS', lowercase_items=True)
        return values 