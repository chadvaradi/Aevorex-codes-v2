...
from modules.financehub.backend.config.environment import EnvironmentSettings
...
class UvicornSettings(BaseModel):
    ...
    model_config = SettingsConfigDict(env_prefix='FINBOT_UVICORN__', env_file='env.local', extra='ignore')
    ...
    def _validate_log_level(cls, v: str) -> str:
        ...
        if level not in valid_levels:
            raise ValueError(f"Invalid Uvicorn LOG_LEVEL: '{level}'. Must be one of {valid_levels}")
        return level
