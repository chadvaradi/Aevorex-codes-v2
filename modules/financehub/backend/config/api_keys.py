...
from modules.financehub.backend.config._core import logger

class APIKeysSettings(BaseSettings):
    """API kulcsok külső szolgáltatásokhoz."""
    ALPHA_VANTAGE: SecretStr | None = Field(default=None, description="Alpha Vantage API kulcs (SecretStr).")
    OPENROUTER: SecretStr | None = Field(default=None, description="OpenRouter API kulcs (SecretStr).")
    MARKETAUX: SecretStr | None = Field(default=None, description="MarketAux API kulcs (SecretStr).")
    ...
    @classmethod
    def _check_api_key_format_if_string(cls, v: str | SecretStr | None) -> str | SecretStr | None:
