...
if "logger_config" in sys.modules:
    ...
    def setup_logging():
        ...
        """
        internal_logger = logging.getLogger("LoggerSetup") # Dedicated logger for the setup process
        ...
        console_handler.setLevel(logging.DEBUG) # Console shows all levels from DEBUG up
        ...
        if LOG_DIR:
            ...
            try:
                ...
                internal_logger.info(f"Added RotatingFileHandler. Log file: {log_file_path}")
            except PermissionError:
                internal_logger.error(f"Permission denied to write to log directory: {log_dir_path}", exc_info=True)
            except Exception as e:
                internal_logger.error(f"Failed to create file handler for {log_dir_path}: {e}", exc_info=True)
        else:
            internal_logger.warning("LOG_DIR environment variable not set. Skipping file logging.")
        ...
        internal_logger.info(f"Logging setup complete. Root logger level: {logging.getLevelName(root_logger.level)}")
