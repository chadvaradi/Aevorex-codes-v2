    ...
    class CacheService:
            ...
            **kwargs
            ...
            if redis_host == "memory":
                ...
                try:
                    ...
                    import importlib
                    import modules.financehub.backend.utils.cache_service as cache_module
            ...
            logger.info(f"[CacheService.create] Creating instance for {redis_host}:{redis_port}")
            ...
            logger.info("[CacheService.create] Successfully created and initialized instance")
            return instance
        ...
        async def initialize(self) -> None:
            ...
            try:
                ...
                try:
                    ...
                    logger.info("[CacheService(Redis)] Applied config: stop-writes-on-bgsave-error=no")
                except Exception as e:
                    logger.warning(f"[CacheService(Redis)] Could not apply config hotfix: {e}")
            ...
            try:
                if self.redis_client:
                    try:
                        ...
                        logger.warning("[CacheService(Redis)] AttributeError during ping – treating as connection lost")
            ...
            try:
                if self.redis_client:
                    try:
                        ...
                        logger.warning(f"[CacheService(Redis)] AttributeError during ping: {attr_err}. Reconnecting...")
                        return False
                ...
                if not self._reconnecting:
                    ...
                    logger.warning("[CacheService(Redis)] Already reconnecting, skipping duplicate attempt")
                    return False
            except Exception as e:
        ...
        async def _reconnect(self) -> None:
            ...
            if self._reconnecting:
                logger.warning("[CacheService(Redis)] Reconnection already in progress, skipping")
                return
        ...
        async def get(self, key: str) -> Optional[str]:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"[CacheService(Redis)] [GET:{key}] Reconnection failed: {reconnect_error}")
                return None
                ...
                try:
                    ...
                    if self.redis_client:
                        if isinstance(value, (dict, list)):
                            ...
                            value_str = str(value)
                        await self.redis_client.setex(key, ttl or self.default_ttl, value_str)
                        return True
                except Exception as reconnect_error:
                    logger.error(f"[CacheService(Redis)] [SET:{key}] Reconnection failed: {reconnect_error}")
                return False
