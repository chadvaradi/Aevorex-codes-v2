...
class SlidingWindowLimiter:
    ...
    async def check_rate_limit(self, identifier: str, limit: int, window: int) -> Tuple[bool, int, int]:
        """
        ...
        try:
            ...
            if not allowed:
                logger.info(f"Rate limit exceeded for {identifier}: {limit} requests per {window}s")
