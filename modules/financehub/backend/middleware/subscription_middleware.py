...
class SubscriptionMiddleware:
    ...
    def __init__(self, subscription_service: Optional[Callable] = None):
        self.subscription_service = subscription_service
    
    async def get_user_subscription(self, user_id: str) -> Optional[UserWithSubscription]:
        """Get user with subscription information."""
        if self.subscription_service:
            return await self.subscription_service(user_id)
        return None
    
    def require_active_subscription(self, min_plan: SubscriptionPlan = SubscriptionPlan.FREE):
    ...
    def _check_plan_access(self, user_plan: SubscriptionPlan, required_plan: SubscriptionPlan) -> bool:
    ...
    async def check_subscription_status(self, user_id: str) -> SubscriptionCheckResponse:
        ...
        return SubscriptionCheckResponse(
            ...
            status=user_with_sub.subscription.status if user_with_sub.subscription else None,
            current_period_end=user_with_sub.subscription.current_period_end if user_with_sub.subscription else None,
            trial_end=user_with_sub.subscription.trial_end if user_with_sub.subscription else None
        )
...
def require_pro_plan():
    ...
    return subscription_middleware.require_plan(SubscriptionPlan.PRO)


def require_team_plan():
    """Require Team plan or higher."""
    return subscription_middleware.require_plan(SubscriptionPlan.TEAM)


def require_enterprise_plan():
    """Require Enterprise plan."""
    return subscription_middleware.require_plan(SubscriptionPlan.ENTERPRISE)


def require_active_subscription():
    """Require any active subscription."""
    return subscription_middleware.require_active_subscription()
