...
class ChatResponse(BaseModel):
    ...
    is_last: bool = Field(default=True, description="Indicates if this is the final message in the stream.")
...
class ChatModelRequest(BaseModel):
    ...
    session_id: str = Field(..., min_length=1, description="Client-side generated session identifier")
    model: str = Field(..., min_length=1, description="Model identifier as returned by /ai/models")


class DeepToggleRequest(BaseModel):
    """Payload to opt-in/opt-out of deep analysis for a specific chat."""
    chat_id: str = Field(..., min_length=1, description="Unique chat id (frontend generated)")
